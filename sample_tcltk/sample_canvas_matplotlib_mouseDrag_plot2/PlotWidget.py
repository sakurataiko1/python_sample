...
class PlotWidget(tk.Canvas):
    ...
    def func_setImage_cv2_fromFile(self, workfilepath):
    ...
    def func_addImage_cv2_fromFile(self, workfilepath, in_xlim, in_ylim):
        # グラフ描画後、画像を重ねて表示する
        # 　　※in_xlim, in_ylim で画像を、グラフ位置に調整して重ねる　この記述がないとずれてしまって重なって見えない場合がある
        image = cv2.imread(workfilepath)
        self.ax.imshow(image, extent=[*in_xlim, *in_ylim], aspect='auto', alpha=0.6)

    def func_addImage_cv2(self, in_image, in_xlim, in_ylim):
        # 引数 in_image はファイルから cv2.imreadで読み込み済みのデータ
        #
        # グラフ描画後、画像を重ねて表示する
        # 　　※in_xlim, in_ylim で画像を、グラフ位置に調整して重ねる　この記述がないとずれてしまって重なって見えない場合がある
        self.ax.imshow(in_image, extent=[*in_xlim, *in_ylim], aspect='auto', alpha=0.6)